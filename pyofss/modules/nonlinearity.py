...
def calculate_gamma(nonlinear_index, effective_area, centre_omega=2.0 * pi * 193.1):
    """
    ...
    gamma = 1.0e24 * nonlinear_index * centre_omega / (effective_area * Domain.vacuum_light_speed)
...
def calculate_raman_term_silica(domain):
    ...
    """
    CP = [56.25, 100.00, 231.25, 362.50, 463.00, 497.00, 611.50, 691.67, 793.67, 835.50, 930.00, 1080.00, 1215.00]
    Ai = [1.00, 11.40, 36.67, 67.67, 74.00, 4.50, 6.80, 4.60, 4.20, 4.50, 2.70, 3.10, 3.00]
    Gfwhm = [52.10, 110.42, 175.00, 162.50, 135.33, 24.50, 41.50, 155.00, 59.50, 64.30, 150.00, 91.00, 160.00]
    ...
    for i in range(13):
        h_R += (
            ...
            * np.exp(-((np.pi * 1e-10 * constants.c * Gfwhm[i]) ** 2) * (t**2) / 4.0)
...
class Nonlinearity(object):
    ...
    def __init__(
        ...
        self.generate_nonlinearity = getattr(self, "%s_nonlinearity" % sim_type, self.default_nonlinearity)

        if use_all:
            print("Using general expression for nonlinearity")
            self.non = getattr(self, "%s_f_all" % sim_type, self.default_f_all)
            self.exp_non = getattr(self, "%s_exp_f_all" % sim_type, self.default_exp_f_all)
        else:
            if self.self_steepening and self.raman_scattering:
                print("Using self_steepening and raman_scattering")
                self.non = getattr(self, "%s_f_with_ss_and_rs" % sim_type, self.default_f_with_ss_and_rs)
                self.exp_non = getattr(self, "%s_exp_f_with_ss_and_rs" % sim_type, self.default_exp_f_with_ss_and_rs)
            elif self.self_steepening:
                print("Using self_steepening")
                self.non = getattr(self, "%s_f_with_ss" % sim_type, self.default_f_with_ss)
                self.exp_non = getattr(self, "%s_exp_f_with_ss" % sim_type, self.default_exp_f_with_ss)
            elif self.raman_scattering:
                print("Using raman_scattering")
                self.non = getattr(self, "%s_f_with_rs" % sim_type, self.default_f_with_rs)
                self.exp_non = getattr(self, "%s_exp_f_with_rs" % sim_type, self.default_exp_f_with_rs)
            else:
                self.non = getattr(self, "%s_f" % sim_type, self.default_f)
                self.exp_non = getattr(self, "%s_exp_f" % sim_type, self.default_exp_f)
    ...
    def __call__(self, domain):
        ...
        if self.use_all is False:
            ...
            raise UnknownRamanResponseError('Use False, True or a sting from the list ["blowwood", "hollenbeck"]')
    ...
    def wdm_exp_f(self, As, h, Bs):
        return np.asarray(
            [
                np.exp(h * self.factor[0] * (np.abs(As[0]) ** 2 + 2.0 * np.abs(As[1]) ** 2)) * Bs[0],
                np.exp(h * self.factor[1] * (np.abs(As[1]) ** 2 + 2.0 * np.abs(As[0]) ** 2)) * Bs[1],
            ]
        )
